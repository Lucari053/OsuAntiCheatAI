...
from parser import slidercalc

# From https://github.com/Awlexus/python-osu-parser/blob/master/beatmapparser.py
...
class BeatmapParser():
    ...
    def parse_hit_object(self, line):
        ...
        if object_type & 1:
            ...
            path = slidercalc.SliderPath(hit_object['curveType'], hit_object['points'])
            hit_object['slider_path'] = path
            end_point = path.point_at_distance(hit_object['pixelLength'])
            if end_point and abs(end_point[0]) < 1e5 and abs(end_point[1]) < 1e-5:
    ...
    def parse_event(self, line):
        ...
        if len(members) >= 3:
            if members[0] == '0' and members[1] == '0' and members[2]:
                ...
                if bg_name.startswith('"') and bg_name.endswith('"'):
                    self.beatmap["bg_filename"] = bg_name[1:-1]
                else:
                    self.beatmap["bg_filename"] = bg_name
            elif members[0] == '2' and members[1].strip().isdigit() and members[2].strip().isdigit():
    ...
    def compute_max_combo(self):
        ...
        for hit_object in self.beatmap["hitObjects"]:
            if hit_object["startTime"] >= next_offset:
                ...
                i += 1
                next_offset = timing_points[i]["offset"] if i < len(timing_points) else math.inf
